...
public class BigBackpackItem extends GameHelperModItem {
	public BigBackpackItem() {
		super("big_backpack");
		setMaxStackSize(1);
	}
	...
	public ItemStack onItemRightClick(ItemStack itemstack, World world, EntityPlayer player) {
		if (!world.isRemote) {
//			GameHelper.getUtils().broadcastString("MSB=" + player.getUniqueID().getMostSignificantBits() + ";LSB="
			...
			if (!player.isSneaking()) {
				if (itemstack.hasTagCompound()) {
					...
					if (!itemstack.getTagCompound().getBoolean("private"))
						player.openGui(GameHelper.instance, GuiHandler.BIG_BACKPACK_GUI, world, 0, 0, 0);
					else {
