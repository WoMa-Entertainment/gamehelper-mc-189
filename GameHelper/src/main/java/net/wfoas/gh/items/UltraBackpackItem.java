...
public class UltraBackpackItem extends GameHelperModItem {
	public UltraBackpackItem() {
		super("ultra_backpack");
		setMaxStackSize(1);
	}
	...
	public ItemStack onItemRightClick(ItemStack itemstack, World world, EntityPlayer player) {
		if (!world.isRemote) {
//			GameHelper.getUtils().broadcastS("MSB=" + player.getUniqueID().getMostSignificantBits() + ";LSB="
			...
			if (!player.isSneaking()) {
				if (itemstack.hasTagCompound()) {
					...
					if (!itemstack.getTagCompound().getBoolean("private"))
						player.openGui(GameHelper.instance, GuiHandler.ULTRA_BACKPACK_GUI, world, 0, 0, 0);
					else {
