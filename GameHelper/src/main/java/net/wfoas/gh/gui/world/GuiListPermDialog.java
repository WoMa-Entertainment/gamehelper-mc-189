...
public class GuiListPermDialog extends GuiScreen implements IGuiPermElement {
	private static final ResourceLocation BACKGROUND = new ResourceLocation("gamehelper", "gui/gui_background.png");
	OverlayTexToggleButton visit, build, destroy, collect_items_exp, kill, interact;
	...
	public void initGui() {
		super.initGui();
		title = I18n.format("gamehelper.viewperm.title");
		...
		playerList = new GuiList(ClientProxy.onlinePlayers,
				...
				new GuiList.ActionListener() {
					...
					public void actionPerformed(GuiList guiList, int slotIndex, boolean isDoubleClick, int mouseX,
						...
						GuiListPermDialog.this.selectedPlayer = guiList.getSelectedString();
						if (GuiListPermDialog.this.selectedWorld == null)
							return;
		...
		worldList = new GuiList(ClientProxy.allWorlds,
				...
				new GuiList.ActionListener() {
					...
					public void actionPerformed(GuiList guiList, int slotIndex, boolean isDoubleClick, int mouseX,
							int mouseY) {
						GuiListPermDialog.this.selectedWorld = guiList.getSelectedString();
						if (GuiListPermDialog.this.selectedPlayer == null)
							return;
						PacketPlayRequestProfileForPlayerInWorld packet = new PacketPlayRequestProfileForPlayerInWorld(
								selectedPlayer, selectedWorld);
						NetworkHandler.sendToServer(packet);
	...
	public void drawScreen(int x, int y, float par3) {
		...
		Minecraft.getMinecraft().fontRendererObj.drawStringWithShadow(t_interact, this.width / 2 + 70 - 60,
				this.height / 2 + 4.5f, 0xffffffff);
//		System.out.println("V" + visit.selected + "D" + destroy.selected + "V" + build.selected + "K" + kill.selected
//				+ "C" + collect_items_exp.selected + "I" + interact.selected);
	...
	public void setProfileForGuiSetPermScreen(boolean visit, boolean destroy, boolean build, boolean kill,
		...
		System.out.println(
				"V" + visit + "D" + destroy + "V" + build + "K" + kill + "C" + collect_items_exp + "I" + interact);
