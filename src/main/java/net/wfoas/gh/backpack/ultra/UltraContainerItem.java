...
public class UltraContainerItem extends Container {
	 ...
	 */
	private static final int INV_START = UltraInventoryItem.INV_SIZE, INV_END = INV_START + 26,
	...
	public UltraContainerItem(EntityPlayer par1Player, InventoryPlayer inventoryPlayer,
			UltraInventoryItem inventoryItem) {
		...
		// model
		for (i = 0; i < UltraInventoryItem.INV_SIZE; ++i) {
			...
			// this.addSlotToContainer(new SlotItemInv(this.inventory, i, 80 +
			// (18 * (int)(i/4)), 8 + (18*(i%4))));
			this.addSlotToContainer(
					new UltraSlotItemInv(this.inventory, i, 8 + (18 * (i % 12)), 18 + (18 * (i / 12))));
		}
		...
		for (i = 0; i < 3; ++i) {
			for (int j = 0; j < 9; ++j) {
				this.addSlotToContainer(new Slot(inventoryPlayer, j + i * 9 + 9, 16 + 3 + 6 + 11 + 8 + j * 18,
						84 + 56 + 2 + 16 + i * 18));
			}
		}
		...
		// texture file
		for (i = 0; i < 9; ++i) {
			this.addSlotToContainer(new Slot(inventoryPlayer, i, 16 + 6 + 3 + 11 + 8 + i * 18, 56 + 16 + 2 + 142));
		}
	}
	...
	public ItemStack transferStackInSlot(EntityPlayer par1EntityPlayer, int index) {
		...
		if (slot != null && slot.getHasStack()) {
			...
			// or armor slots
				...
				// {
				// if (!this.mergeItemStack(itemstack1, INV_START, INV_END+1,
				// false))
				// {
				// return null;
				// }
