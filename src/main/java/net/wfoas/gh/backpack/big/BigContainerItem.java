...
public class BigContainerItem extends Container
{
	...
	private static final int INV_START = BigInventoryItem.INV_SIZE, INV_END = INV_START+26,
	...
	public BigContainerItem(EntityPlayer par1Player, InventoryPlayer inventoryPlayer, BigInventoryItem inventoryItem)
	{
		...
		for (i = 0; i < BigInventoryItem.INV_SIZE; ++i)
		{
...
//			this.addSlotToContainer(new SlotItemInv(this.inventory, i, 80 + (18 * (int)(i/4)), 8 + (18*(i%4))));
			this.addSlotToContainer(new BigSlotItemInv(this.inventory, i, 8 + (18 * (i%9)), 18 + (18*(i/9))));
		}
		...
		*/
		...
		for (i = 0; i < 3; ++i)
		{
			for (int j = 0; j < 9; ++j)
			{
				this.addSlotToContainer(new Slot(inventoryPlayer, j + i * 9 + 9, 11 + 8 + j * 18, 84 + 56 + i * 18));
			}
		}
		...
		for (i = 0; i < 9; ++i)
		{
			this.addSlotToContainer(new Slot(inventoryPlayer, i, 11 + 8 + i * 18, 56 + 142));
		}
	}
